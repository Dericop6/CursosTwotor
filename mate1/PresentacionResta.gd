...
func _ready():
	...
	if main_node == null:
		print("Error: No se pudo encontrar el nodo 'Main' en PresentacionResta.gd.")
		return
	...
	if btn_practicar:
		btn_practicar.pressed.connect(func():
			print("Navegando a PracticaResta.tscn...")
			# Pide a main_node que cargue la escena de práctica
			main_node.load_downloaded_scene(CURRENT_COURSE_NAME, "PracticaResta.tscn")
		)
	else:
		print("Error: Botón BtnPracticar no encontrado en PresentacionResta.gd. Revisa la ruta.")
	...
	if btn_retroceder:
		btn_retroceder.pressed.connect(func():
			print("Volviendo a la pantalla de Cursos desde PresentacionResta.gd.")
			# Pide a main_node que cargue la pantalla de cursos
			main_node.cargar_pantalla("cursos")
		)
	else:
		print("Error: Botón BtnRetroceder no encontrado en PresentacionResta.gd. Revisa la ruta.")
